...
XDL_ACCEPTABLE_UNITS = {
    ...
    'mol': ['umol', 'mmol', 'mol'],
    'time': ['s', 'sec', 'secs', 'second', 'seconds', 'm', 'min', 'mins', 
...
class XDLSyntaxValidator(object):
    ...
    def all_reagents_declared(self):
        ...
        """
        declared_reagent_ids = [reagent.attrib['rid'] 
    ...
    def all_vessels_declared(self):
        ...
        """
        declared_vessel_ids = [component.attrib['cid'] 
        ...
        for step in self.steps:
            ...
            if step.tag in XDL_STEP_COMPULSORY_ATTRIBUTES:
                ...
                for attr in XDL_STEP_COMPULSORY_ATTRIBUTES[step.tag]:
                    # 'quantity' wildcard used if mass and volume are both 
                    # acceptable, but one of them must be present.
                    if attr == 'quantity':
                        has_quantity = False
